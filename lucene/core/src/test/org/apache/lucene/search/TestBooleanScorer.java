...
public class TestBooleanScorer extends LuceneTestCase {
  ...
  // Regression test: a SHOULD-only BooleanQuery with TOP_SCORES and
  // minShouldMatch > 1 must not be dispatched to BooleanScorer, which lacks
  // top-K impact pruning. booleanScorer() should return null so dispatch
  // falls back to BS2/WANDScorer, which supports both TOP_SCORES pruning
  // and minShouldMatch >= 2.
  public void testTopScoresWithMinShouldMatchFallsBackToWand() throws IOException {
    ...
    // matches are dense; unchanged by the fix.
    weight = searcher.createWeight(searcher.rewrite(query), ScoreMode.COMPLETE, 1);
    ss = weight.scorerSupplier(ctx);
    assertTrue(
